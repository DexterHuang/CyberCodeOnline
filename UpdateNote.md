...
- Updated filter list `queer` removed from the filter list, added "arc****t" and "cco2" to filter list, as I noticed there are rumors spreading for CCO closing down due some nonsensical reasons, I would like to take this opportunity to refute those rumors to avoid further misinformation:
  ...
  - **Are you gonna change the monetization strategy of CCO?** No, the is no plan to do that, we have kept it for so long, that I think I would rather see it closing down (which hopefully wont happen for a long time :D) due to lack of donations than change it to a P2W system for CCO. I hope if CCO closes down it will be because I make a CCO2 one day 😃
