...
チャレンジダンジョンのボスは撃破時に貰える経験値がとても多い代わりに, 体力と攻撃力が段違いに高くなっています.  
「同レベルのノーマルダンジョンを普通にクリアできたから」ぐらいの感覚で挑むと, 下手をすれば一撃で心停止です. クリアに手こずる場合は他のプレイヤーに協力してもらうのもよいでしょう.


## ダンジョンマルチプレイに関する用語集

ダンジョン用語や略語は, ダンジョンを複数人でプレイする際にプレイヤーがダンジョン内で目標（例：ボスルームやその中にいるボス）に効率よく向かうために仲間とコミュニケーションを取る方法のひとつです.

以下に主にダンジョンチャットでよく使用されるダンジョンマルチプレイのための略語を記載します.
...
| 用語 | 説明 |
|--|--|
| 0-3 | （入ってきたドアを除く）ダンジョン内のドアの数を示す |
| U/D/L/R or ^/v/</> | 上下左右の方向を示す. 単独で使う場合は次に向かうべきドアの方向を伝える|
| 0-3 + U/D/L/R or ^/v/</> | 指定した方向のルームのドアの数を示す（例：0R → 右側のドアの先は行き止まりだ） |
| 7/BR | 現在の場所に宝物庫／ボスルームがあることを示す |
| 8/9/88/B | ボスを見つけたことを示す. 基本すべて同じ意味だが, 同一人物が続けて発言した際の区別として使い分けられる |
| 8/9/88/B + U/D/L/R or ^/v/</> | ボス発見の際に複数の敵と隣接している場合に, 隣接しているボスの位置を示す（例：8> → 右側に接している敵がボスだ） |
| door | ドアの前のボスを見つけたことを示す（ドアの前で動かない状態で, ラグなのかボスと対面しているか判別するための使い分け） |
| 20/+/x/. | ボス報酬取得条件（与ダメージ20%以上）を満たしたことを示す |
| 34/M/Minigame | 現在ミニゲームに挑戦中であることを示す |
| 😇/I died |「心停止しました」 |
| can't see/lag | 仲間の位置情報が全く更新されない（ラグの可能性がある）ことを伝える |
| omw |「今向かってます」 |
| brt |「すぐ到着します」 |
| s/sell |「NPCに装備を売ってきます」（インベントリを空けるため） |


### 休憩したいとき

マルチプレイ中に休憩を挟みたい時は, 以下のような表現を使います.
...
マルチプレイを終了したい時は「○○ ty」（"ty"は Thank youの略）のように終わりにする理由を添えて感謝の意を添えて終了とする表現が使われます.  
「○○」に入る終了の理由としては afk, skip, level up, sleep￤afkタスク入れるから, スキップ来てるから, レベルが上がったから, 寝るから などがあります.  
特段の理由がないときは「rest ty(終わります, ありがとう)」のように宣言して終わります.


### その他のマルチプレイに関するTips

- ボス戦に限らず, 敵と戦う際は2体以上の隣接を避けるように行動すると, どれと対面しているか分かりやすく仲間もスムーズに動けるため推奨されます.
- 同一の敵へのバッティングを避ける為に, 部屋に入りすぐに戦わず敵の近くに寄るだけで様子見したりするケースもあります（ラグで仲間が瞬間移動する事が多いため. 仲間の位置情報の更新待ちのような場合）.
- マルチプレイは通常3人までの人数で行われます. 4人以上はボス報酬の取得条件を見誤るケースが増えるため基本行われません.

これらの用語は他の表現をされるケースも多いため, 実際には一緒に潜る仲間と意図が伝わってるか確認しながらマルチプレイを楽しんでください！
