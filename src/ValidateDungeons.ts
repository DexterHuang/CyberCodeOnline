        ...
        for (const dungeon of dungeons) {
            try {
                ...
                if (dungeonMap.has(key)) {
                    errors.push({type: type, roomNumber: index, message: `Duplicate found! room number ${index} is a duplicate of room number ${dungeonMap.get(key)}!`, dungeon: dungeon});
                }
                else {
                ...
                errors.push({type: type, roomNumber: index, message: e.message, dungeon: dungeon});
            }
    ...
    private validateGenKey(dungeon: Dungeon) : string {
        ...
        for (let i = 0; i < dungeon.tiles.length; ++i) {
            ...
            if (this.isBorder(x, y) && !value) {
                throw new Error(`Border is not correct!`);
            }
            if (this.isDoor(x, y) && value) {
                throw new Error(`Doors are not correct!`);
            }
            layout[x][y] = value;
        }
